...
def calculate_price(gallons, time_of_day):
    ...
    if 0 <= time_of_day < 6:  # Off-peak hours
        rate = RATE_SINGLE_FAMILY_FIRST_TIER
    elif 6 <= time_of_day < 18:  # Peak hours
...
st.write("Calculate the cost of water usage based on the number of gallons and the time of day.")

# User input for gallons and time of day
gallons = st.number_input("Enter the number of gallons:", min_value=1, value=300)
time_of_day = st.slider("Enter the time of day (0-23):", min_value=0, max_value=23, value=8)
